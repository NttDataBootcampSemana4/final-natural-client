package com.bootcampfinal.naturalclient.app.controllers;
...
import com.bootcampfinal.naturalclient.app.services.NaturalClientService;
import com.bootcampfinal.naturalclient.app.services.TypeNaturalClientService;
...
public class NaturalClientController {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(NaturalClientController.class);

	@Value("${config.balanceador.test}")
	private String balanceadorTest;

	@Autowired
	private NaturalClientService service;
	...
	@GetMapping("/all")
	public Flux<NaturalClient> searchAll() {
		Flux<NaturalClient> nat = service.findAlls();
	...
	@GetMapping("/id/{id}")
	public Mono<NaturalClient> searchById(@PathVariable String id) {
		LOGGER.info("NATURAL LEGAL CLIENT id: " + service.findById(id) + " con codigo: " + id);
		return service.findById(id);
	}

	@PostMapping("/create-natural-client")
	public Mono<NaturalClient> createNaturalLegalClient(

		@Valid @RequestBody NaturalClient naturalLegalCLient) {
	...
	public ResponseEntity<Mono<?>> newNaturalPerson(@PathVariable String id,
			@Valid @RequestBody NaturalClient naturals) {
		...
		Mono<NaturalClient> newNaturalPerson = service.saves(naturals);
		...
		return new ResponseEntity<>(Mono.just(new NaturalClient()), HttpStatus.I_AM_A_TEAPOT);
	}
	...
	public ResponseEntity<Mono<?>> updateTypeNaturalPerson(@Valid @RequestBody TypeNaturalClient type,
		...
		List<NaturalClient> naturals = new ArrayList<>();
		Flux<NaturalClient> fluxnatural = Flux.fromIterable(naturals);
		Flux<NaturalClient> fx2 = fluxnatural;
	...
	public ResponseEntity<Mono<?>> updateNaturalPerson(@Valid @RequestBody NaturalClient naturals,
		...
		Mono<NaturalClient> newNaturalPerson = service.findById(naturalClientId);
	...
	public ResponseEntity<Mono<Void>> deleteNaturalPerson(@PathVariable String id) {
		NaturalClient natural = new NaturalClient();
		natural.setId(id);
		Mono<NaturalClient> newNaturalPerson = service.findById(id);
